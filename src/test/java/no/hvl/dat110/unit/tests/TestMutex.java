...
class TestMutex {
	...
	void test() throws InterruptedException, RemoteException {
		...
		// request to update the same file will happen concurrently - only one process
		// should get the vote
		FileUpdater fu1 = new FileUpdater(p1, getPeerMessage(activepeers, p1.getNodeName()), newupdate.getBytes(),
				activepeers);
		FileUpdater fu2 = new FileUpdater(p2, getPeerMessage(activepeers, p2.getNodeName()), newupdate.getBytes(),
				activepeers);
		FileUpdater fu4 = new FileUpdater(p4, getPeerMessage(activepeers, p4.getNodeName()), newupdate.getBytes(),
				activepeers);
		FileUpdater fu5 = new FileUpdater(p5, getPeerMessage(activepeers, p5.getNodeName()), newupdate.getBytes(),
				activepeers);
		...
		System.out.println("Expected: " + Arrays.toString(e.toArray()));
		System.out.println("Actual  : " + Arrays.toString(replies.toArray()));
