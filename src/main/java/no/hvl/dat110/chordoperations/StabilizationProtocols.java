...
public class StabilizationProtocols extends TimerTask {
	...
	/**
	 * This protocol is not in the original paper but it is crucial for stabilizing
	 * the chord ring
	 * periodically, a node n tries to resolve the next key (i.e. succ id which is
	 * nodeid + 1) from itself
	 * node.findsuccessor(node+1). The result is the node ns that's still active and
	 * responsible for nodeid+1
	...
	private void updateSuccessor() {
		try {
			...
			NodeInterface succnodestub = chordnode.findSuccessor(succid); // finds the successor (succ(nodestub+1) of
																			// this node(remote call)
			...
			try {
				if (chordnode.getNodeName().equals(predsucc.getNodeName())) {
					...
					((Node) chordnode).getFingerTable().set(0, predsucc); // update the first successor (entry) of the
																			// finger table
				...
				// in the case of a two ring node: we need this mechanism to eventually update
				// the predecessors
	...
	private void printInfo() {
		try {
			...
			System.out
					.println("successor(" + chordnode.getNodeName() + ") = " + chordnode.getSuccessor().getNodeName());
			if (chordnode.getPredecessor() == null)
				System.out.println("predecessor(" + chordnode.getNodeName() + ") = " + chordnode.getNodeName());
			else
				System.out.println(
						"predecessor(" + chordnode.getNodeName() + ") = " + chordnode.getPredecessor().getNodeName());
			// System.out.println("Current FingerTable for "+chordnode.getNodeIP()+" =>
			// "+Util.toString(chordnode.getFingerTable()));
