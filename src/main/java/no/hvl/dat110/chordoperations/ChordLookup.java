...
public class ChordLookup {
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		List<NodeInterface> fingers = ((Node) node).getFingerTable();
		...
		for (int i = fingers.size() - 1; i >= 0; i--) {
			...
			boolean condition = Util.checkInterval(finger.getNodeID(), node.getNodeID().add(BigInteger.ONE),
		...
		// if no finger is found, return the current node as the highest predecessor
